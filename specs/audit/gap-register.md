...
| G-014 | High | Readiness | `--config-check` validates shapes and notification presence, then says “ready to run” without reaching the database, Redis, Polygon, or trade source. | `__main__.py` | **Closed by slice 003**: `--config-check` clarified as offline syntax/structure validation; live readiness separated into `/ready` HTTP endpoint probing database, Redis, and ingestion; see [verification](../003-safe-observable-operation/evidence/verification.md). |
