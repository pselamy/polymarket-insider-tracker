...
- The CI matrix provides executable evidence for Python 3.11, 3.12, and 3.13. Ruff, mypy, and Pyright
  ...
  complete production package in strict mode. Vulture checks for dead code, and Complexipy checks cognitive
  complexity across all tracked Python files with a maximum allowed score of 5 and `--no-ignore` enabled.
...
|---|---|---|
| `static` | lock, format, lint, strict-types, pyright, vulture, complexipy | None |
...
| Field | Type | Rule |
|---|---|---|
| `id` | stable enum | `lock`, `format`, `lint`, `strict-types`, `pyright`, `vulture`, `complexipy`, `imports`, `tests`, `services`, `migrations` |
