...
# eth_getLogs block-range chunking. Provider caps vary widely on free tiers:
# publicnode/ankr/llamarpc nominally allow 10_000 blocks, but drpc's free tier
# rejects any range > 1_000 (its error message says "10000" but empirically
# anything over 1_000 fails with "ranges over 10000 blocks are not supported
# on freetier"). We size to the smallest known cap so the same chunk works
# on every provider in the failover chain.
DEFAULT_CHUNK_SIZE_BLOCKS = 1_000
