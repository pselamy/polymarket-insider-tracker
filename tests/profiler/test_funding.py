...
class TestGetTransferLogs:
    ...
    async def test_get_transfer_logs_formats_topics_correctly(
        ...
        # The chunk bounds match what we asked for.
        assert call_args["fromBlock"] == 1
        assert call_args["toBlock"] == 1_000
    ...
    async def test_get_transfer_logs_chunks_large_ranges(
        ...
        # 2_500 blocks at 1_000-per-chunk → 3 calls (1000 + 1000 + 501).
        await funding_tracer._get_transfer_logs(
            to_address=TEST_WALLET,
            token_address=USDC_BRIDGED,
        ...
        assert windows[0]["toBlock"] == 1_000_999
        assert windows[1]["fromBlock"] == 1_001_000
        assert windows[1]["toBlock"] == 1_001_999
        assert windows[2]["fromBlock"] == 1_002_000
        assert windows[2]["toBlock"] == 1_002_500
        # No window exceeds the chunk size — that's what RPC providers reject.
        for win in windows:
            assert win["toBlock"] - win["fromBlock"] + 1 <= 1_000
